...
function AddChemist() {
    ...
    const addChemistFunction = async () => {
        if (chemist.name === "" || chemist.address === "")
            return toast.error("Name and Address are Mandatory Fields")

        try {
            const chemistRef = collection(db, 'chemist')
            await addDoc(chemistRef, chemist)
            toast.success("Chemist Added Successfully")
            navigate('/chemistdetails')
        } catch (error) {
            console.error("Error in adding chemist" , error)
    ...
    return (
        <>
            <h1 className="text-center font-bold text-5xl text-green-700 mt-10 mb-10">Add Chemist</h1>
